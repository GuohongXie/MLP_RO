...
def replace_Q_with_u(source_csv_path, target_csv_path, all_info_path, source_csv_list, print_log_path):
    ...
    section_begin  = 0
    section_end    = 0
    df_source['Qtot'][0] = u_list[source_csv_list[cnt-1]-1]

    for i in range(1, len(df_source)):
        if(abs((df_source['Qtot'][i] - df_source['Qtot'][i-1])) > 1e-4):

            print("now replace the %dth Qtot with u"%cnt)
            with open(print_log_path, "a") as log_file:
                print("now replace the %dth Qtot with u"%cnt, file=log_file)
                log_file.close()

            # now compute the cnt_th section
            section_end = i
            u_value = u_list[source_csv_list[cnt-1]-1]
            df_source['Qtot'][section_begin:section_end] = u_value
            ...
            if(cnt == len(source_csv_list)):
                ...
                with open(print_log_path, "a") as log_file:
                    ...
                    log_file.close()
                u_value = u_list[source_csv_list[cnt-1]-1]
                df_source['Qtot'][section_begin:] = u_value
                break
        else:
            continue
